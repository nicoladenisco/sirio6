 ...
 * @param <T> un oggetto generato da Torque
 */
public class TableCacheSec<T extends ColumnAccessByName> implements Iterable<T>
{
  private Class cls;
  private final Class clsObject;
  private String tableName;
  private static final String TABLE_CACHE_CLASS = "TableCache";

  /**
   * Costruisce selettore dei dati.
   * @param cls classe dell'oggetto di ritorno
   */
  public TableCacheSec(Class<T> cls)
  {
    this.clsObject = cls;
  }
     ...
     throws Exception
  {
    if(tableName == null)
    {
      String clsPeerName = clsObject.getName() + "Peer";
      this.cls = Class.forName(clsPeerName);
